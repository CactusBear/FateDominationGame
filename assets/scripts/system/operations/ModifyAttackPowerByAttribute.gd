...
#类别由调用方声明，不在本操作里写死哪张卡该受限
func exec(required_attributes:Array, power_delta:BaseNumber = BaseNumber.new(0), player_id:int = -1, attack:BaseAttack = null, exclude_attack:BaseAttack = null, required_category:String = ""):
	...
	if attack != null:
		...
		if _match(attack, required_attributes, required_category) and counts_power.exec(attack, id):
		...
		for card in player_data["played_cards"]:
			if card is BaseAttack and _match(card, required_attributes, required_category) and counts_power.exec(card, id):
...
func _match(attack:BaseAttack, required_attributes:Array, required_category:String = "") -> bool:
