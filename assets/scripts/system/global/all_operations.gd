...
const TABLE := {
	...
	"get_active_players_id" : { "category": QUERY_PLAYER, "class": "GetActivePlayersId", "summary": "当前顺位链中未出局玩家id" },
	...
	"get_effective_location_benefit" : { "category": QUERY_MAP, "class": "GetEffectiveLocationBenefit", "summary": "当前实际享有的地利(部署才算)" },
	"get_player_location_benefit" : { "category": QUERY_MAP, "class": "GetPlayerLocationBenefit", "summary": "玩家所在地印刷地利数" },
	"is_engaged" : { "category": QUERY_MAP, "class": "IsEngaged", "summary": "是否处于交战状态(与对手同处会发生战斗的战场)" },
	...
	"get_move_target_location" : { "category": QUERY_MAP, "class": "GetMoveTargetLocation", "summary": "常规移动落点(满了返回null)" },
	...
	"is_true_name_released" : { "category": QUERY_LOG, "class": "IsTrueNameReleased", "summary": "查询当前真名是否解放" },
	...
	"get_area_round_participants" : { "category": QUERY_LOG, "class": "GetAreaRoundParticipants", "summary": "本回合某战区实际参战者名单" },
	...
	"remove_from_board" : { "category": EDIT_MAP, "class": "RemoveFromBoard", "summary": "把御主移出版图(回合结束)" },
	...
	"hide_true_name" : { "category": ACTION, "class": "HideTrueName", "summary": "隐藏真名并盖回技能牌" },
